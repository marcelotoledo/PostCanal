$(document).ready(function() { var active_request = false;  function showSpinner() { $.ab_spinner ({ height: 32, width: 32, image: "<?php $this->img_src('spinner/linux_spinner.png') ?>", message: "... carregando" }
 ...
 $.ajax ({ type: "POST", url: "<?php echo $this->url('profile', 'recovery') ?>", dataType: "text", data: parameters, beforeSend: function() { active_request = true; showSpinner(); }
...
, error: function () { $.ab_alert("ERRO NO SERVIDOR"); }
 ...
 $.ajax ({ type: "POST", url: "<?php $this->url('profile') ?>/" + action, dataType: "text", data: parameters, beforeSend: function () { active_request = true; showSpinner(); }
, complete: function() { active_request = false; hideSpinner(); }
